...
public class CH583OTAManager {
    ...
    public int getHexFileEraseAddr(@NonNull File file) throws CH583OTAException {
    ...
    /**
     *
     * @param eraseAddr BIN文件擦除地址有用户自己输入
     *                  HEX文件需从调用getHexFileEraseAddr(File)方法获取擦除地址
     * @param file 升级文件
     * @param currentImageInfo 当前硬件信息
     * @param progress 回调
     * @throws IOException
     * @throws CH583OTAException
     */
    @RequiresPermission(anyOf = {Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE})
    public void start(int eraseAddr,@NonNull File file, @NonNull CurrentImageInfo currentImageInfo, IProgress progress) throws IOException, CH583OTAException {
        stopFlag=false;
        ...
        LogUtil.d("当前芯片类型： "+currentImageInfo.getChipType().toString());
        //image信息
        ...
        if(file.getName().endsWith(".bin") || file.getName().endsWith(".BIN")){
            ...
            throw new CH583OTAException("only support hex and bin image file");
        }
        if(byteBuffer==null){
    ...
    @Deprecated
    @RequiresPermission(anyOf = {Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE})
    public void start(@NonNull File file, @NonNull CurrentImageInfo currentImageInfo, IProgress progress) throws IOException, CH583OTAException {
        stopFlag=false;
        this.progress=progress;
        //image信息
        LogUtil.d("开始升级固件： "+file.getAbsolutePath());
