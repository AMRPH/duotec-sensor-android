...
public class FileParseUtil {
    ...
    public static ByteBuffer parseBinFile(@NonNull File file) throws CH583OTAException, IOException {
    ...
    @RequiresPermission(anyOf = {Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE})
    public static ByteBuffer parseHexFile(@NonNull File file) throws CH583OTAException {
        ...
        try {
            ...
            while ((hexStr=reader.readLine())!=null){
                ...
                if(!hexStr.startsWith(":",0)){
                    throw new CH583OTAException("hex file don't start with ':' ");
                }
                if(!hexStringValid(hexStr.substring(1))){
                    throw new CH583OTAException("hex file contains invalid char ");
                }
                if(hexStr.length()<11){
                    throw new CH583OTAException("hex file content is invalid,every line should't less than 11 ");
                }
                ...
                if(type==0x00){//本行的数据类型为“数据记录”
                    if(fileStruct.getFormat()==0x00){//本行所从属的数据类型为“数据记录”
                        ...
                        throw new CH583OTAException("invalid type!");
                    }
                    //记录地址中的最大最小值
                    ...
                    //扩展段地址记录的数据个数一定是0x02
                    if (fileStruct.getLength() != 0x02) throw new CH583OTAException("incorrect length!");
                    //扩展段地址记录的地址一定是0x0000
                    if (fileStruct.getOffset() != 0x0000) throw new CH583OTAException("incorrect address!");
                    //更改从属的数据类型
                    ...
                    throw new CH583OTAException("this type is undefined");
                }
     ...
     * @param file
     * @return
     * @throws CH583OTAException
     */
    @RequiresPermission(anyOf = {Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE})
    public static int parseHexFileStartAddr(@NonNull File file) throws CH583OTAException {
        if(file==null || !file.exists() || !file.isFile() ||(!file.getName().endsWith("hex") && !file.getName().endsWith("HEX"))){
            throw new CH583OTAException("this file don't exist or is invalid");
        }
        long length = file.length();
