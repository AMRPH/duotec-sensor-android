...
class RelayWorker(private val context: Context, workerParams: WorkerParameters) :
    Worker(context, workerParams) {

    companion object {
        val BR_NAME = "RW_NAME"
        val BR_VALUE = "RW_VALUE"
    }
    ...
    fun subscribeData() {
        if (device.isConnected) {
            App.connection!!
                .setupNotification(UUID.fromString(SensorParams.RELAY_DATA.uuid))
                .flatMap { t: Observable<ByteArray> -> t }
                .subscribe({
                    val dData = RelaySensorData(it)
                    Sensor.relayCacheData = dData
